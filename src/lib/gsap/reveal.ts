...
const REVEAL_SCROLL_START = "top 88%";

export function prefersReducedMotion() {
  return window.matchMedia("(prefers-reduced-motion: reduce)").matches;
}

function resetRevealElements(elements: gsap.TweenTarget) {
  gsap.set(elements, { autoAlpha: 1, y: 0 });
}

function hideRevealElements(elements: gsap.TweenTarget) {
  gsap.set(elements, { autoAlpha: 0, y: REVEAL_Y });
}

function revealOnMount(elements: gsap.TweenTarget) {
